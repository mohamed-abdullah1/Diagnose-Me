...
const MainQuestions = ({ navigation }) => {
  ...
  const handleSubmitQuestion = () => {
    ...
    ask({ token, body: { questionString: input, tags: [specialty] } });
    setPage(1);
    setQSpecialty("");
    setSearchQuery("");
    //DONE: add the speciality to the body
  };
  ...
  return (
    <BgContainer>
        ...
        {items
          ...
          ?.map((s) => (
            <TouchableOpacity
              key={s.key}
              onPress={() => {
                setQSpecialty(s.value === "all" ? "" : s.value);
                setPage(1);
              }}
            >
              <CategoryItem
                category={s.value}
                current={qSpecialty === "" ? "all" : qSpecialty}
              >
                {s.value}
              </CategoryItem>
            </TouchableOpacity>
      ...
      >
        ...
        ) : questions?.objects.length > 0 ? (
          <>
            {questions?.objects?.map((q) => (
              <QuestionCard
                onPress={() => {
