...
const RegistrationBloodType = ({ navigation }) => {
  ...
  const userData = useSelector(selectRegisterUser);
  ...
  useEffect(() => {
    if (registerIsError) {
      ...
      Alert.alert(
        ...
        `Error:- ${registerError?.data?.errors}`,
        [
          {
            text: "Try again",
          },
    ...
    if (registerSuccess) {
      ...
      Alert.alert(
        ...
        [
          {
            text: "Next",
            onPress: () => navigation.navigate("RegistrationPinCode"),
          },
        ],
        { cancelable: true }
      );
  ...
  return (
    <Background>
      ...
      <Top widthDesc={250} title="Blood Group" desc="Most Doctors Need it" />
      <BloodGroup>
        <BloodItemsContainer>
          <FlatList
            data={bloodGroups}
            renderItem={({ item }) => (
              <Item setBlood={setBlood} item={item} blood={blood} />
            )}
      ...
      <VectorBgContainer>
        <VectorImg source={require("../../../../assets/helpers/bg.png")} />
      </VectorBgContainer>
    </Background>
  );
};
